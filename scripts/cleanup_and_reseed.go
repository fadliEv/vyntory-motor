...
func main() {
	...
	fmt.Println("🧹 Cleaning up bad data...")

	// Delete transactions with empty motor_nama (bad data)
	result, _ := db.Exec("DELETE FROM transactions WHERE motor_nama IS NULL OR motor_nama = ''")
	affected, _ := result.RowsAffected()
	fmt.Printf("  Deleted %d bad transactions\n", affected)

	// Delete motors from 2025 that are not in transactions
	result, _ = db.Exec(`
		DELETE FROM motors
		WHERE tanggal_masuk >= '2025-01-01'
		AND status = 'tersedia'
		AND id NOT IN (SELECT motor_id FROM transactions)
	`)
	affected, _ = result.RowsAffected()
	fmt.Printf("  Deleted %d unused motors\n", affected)

	fmt.Println("\n✅ Cleanup complete!")
	fmt.Println("\n🌱 Inserting fresh data...")

	// Insert 400 new motors
	sqlMotors := `
