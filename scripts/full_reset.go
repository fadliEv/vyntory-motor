...
func main() {
	...
	fmt.Println("🗑️  Full cleanup - removing ALL 2025 data...")

	// Delete ALL transactions with INV/2025/*
	result, _ := db.Exec("DELETE FROM transactions WHERE invoice_number LIKE 'INV/2025/%'")
	affected, _ := result.RowsAffected()
	fmt.Printf("  Deleted %d transactions (2025)\n", affected)

	// Delete ALL motors from 2025
	result, _ = db.Exec("DELETE FROM motors WHERE tanggal_masuk >= '2025-01-01'")
	affected, _ = result.RowsAffected()
	fmt.Printf("  Deleted %d motors (2025)\n", affected)

	fmt.Println("\n✅ Full cleanup complete!\n")

	// Now insert fresh data
	fmt.Println("🌱 Inserting 400 fresh motors...")
...
`
	...
	// Get motors for transactions
	fmt.Println("💰 Creating transactions (Jan-Oct 2025)...")
	...
	for month := 1; month <= 10; month++ {
		...
		fmt.Printf("  ✓ Month %d: %d transactions\n", month, numTrx)
	}
...
DONE:
	...
	fmt.Println("📊 Final Database Summary:")
